...
public class IncomingSms extends BroadcastReceiver {
        ...
        try {
            if (bundle != null) {
                ...
                for (int i = 0; i < pdusObj.length; i++) {
                    ...
                    if (recentMessage(context.getApplicationContext(), senderNum, autoReplyMessage, offset)) {
                        return;
                    }
    ...
    public static final boolean recentMessage(Context _activity, String number, String msg, int offset) {
        long inboxMsgDate = 0;
        long sentMsgDate = 0;
        Uri lookupUri = Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, Uri.encode(number));
        ...
        if (cursor.moveToFirst()) { // must check the result to prevent exception
            do {
                ...
                if (cursor.getString(cursor.getColumnIndex("body")) != msg) {
                    duplicate = false;
                }
            } while (cursor.moveToNext() && duplicate);
        }
